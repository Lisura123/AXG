...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  useEffect(() => {
    if (user && product.id) {
      const inWishlist = localIsInWishlist(user._id, product.id);
  ...
  const handleToggleWishlist = (e: React.MouseEvent) => {
    ...
    if (variant === "wishlist" && onRemoveFromWishlist) {
      ...
      try {
        const result = localToggleWishlist(user._id, product.id);
  ...
  const productImage = getImageUrl(product.image_url) || placeholderSvg;
      ...
      >
        <div>
          ...
          <div className="flex items-center justify-between mb-3">
            {variant !== "featured" && product.is_featured && (
